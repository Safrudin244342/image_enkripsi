...
class setting_gui_main():
    def __init__(self,root):
        self.root = root
        self.root.title("Enkripsi Image 2.0")
    ...
    def set_gui(self):
        ...
        self.en_input_text = Label(self.root,text="Cari gambar yang akan di enkripsi")
        self.en_input_text.place(x=90,y=15)
        # membuat kolom input file
        Button(self.root, text="Search",command=self.save_img_file).place(x=10, y=50)
        self.en_save_text = Label(self.root, text="Lokasi file enkripsi akan disimpan")
        ...
        Button(self.root,text="Enkripsi",command=self.enkripsi_img).place(x=150, y=120)
    ...
    def input_img_file(self):
        self.en_input_file = askopenfilename(filetypes=(("PNG","*.png"),("JPG","*.jpg"),("JPEG","*.jpeg")))
    ...
    def enkripsi_img(self):
        ...
        if len(self.en_save_file) > 0 and len(self.en_input_file) > 0 and len(self.txt_password.get()) > 0:
            ...
            t = threading.Thread(target=self.enkripsi_img_proses, args=(img, password))
            t.start()
    ...
    def enkripsi_img_proses(self,img,password):
        # deklarasi semua variable
        data_pixel = []
        # membuat gambar enkripsi
        ...
        for lebar in range(width - 1):
            ...
            for tinggi in range(height - 1):
                if i <= ord_huruf:
                    ...
                    if img.mode == "RGB":
                        x, y, z = img.getpixel((lebar, tinggi))
                    elif img.mode == "RGBA":
                        x, y, z, _ = img.getpixel((lebar, tinggi))

                    # enkripsi setiap pixel
                    hash = self.rand_enkripsi(x, y, z, nilai_warna)
        ...
        # memasukkan password kedalam gambar
        sha = sha512()
        sha.update(password.encode("utf-8"))
        lsb.hide(self.en_save_file, sha.hexdigest()).save(self.en_save_file)

        showinfo("Pemberitahuan","Gambar telah di enkripsi, tersimpan di\n" + self.en_save_file)
    ...
    def rand_enkripsi(self,x,y,z,nilai_warna):
        x_hasil = x
        y_hasil = y
        z_hasil = z
        if nilai_warna == "R":
        ...
        hash = (x_hasil, y_hasil, z_hasil, a_hasil)
        return hash
